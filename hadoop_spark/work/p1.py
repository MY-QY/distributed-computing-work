...
# 计算每个学生必修课的平均成绩
student_avg_scores = rdd.filter(lambda row: row[3] == "必修").map(lambda row: ((row[0], row[1]), row[4])) \
    .groupByKey().mapValues(lambda scores: sum(scores) / len(scores))

# 按科目统计每个班的平均成绩
class_subject_avg_scores = rdd.map(lambda row: ((row[0], row[2]), row[4])) \
    .groupByKey().mapValues(lambda scores: sum(scores) / len(scores))
